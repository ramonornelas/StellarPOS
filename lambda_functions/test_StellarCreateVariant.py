import json
from StellarCreateVariant import lambda_handler
...
if __name__ == "__main__":
    print("Testing StellarCreateVariant with TEST environment only...")
    ...
    test4 = test_missing_product_id()
    test5 = test_invalid_json()
    
    print("\n" + "=" * 60)
    print("Test Summary:")
    print(f"TEST Environment: {'✓ Success' if test1 and test1.get('statusCode') in [201, 500] else '✗ Failed'}")
    print(f"No Context (PROD): {'✓ Success' if test2 and test2.get('statusCode') in [201, 500] else '✗ Failed'}")
    print(f"Missing Fields: {'✓ Success' if test3 and test3.get('statusCode') == 400 else '✗ Failed'}")
    print(f"Missing Product ID: {'✓ Success' if test4 and test4.get('statusCode') == 400 else '✗ Failed'}")
