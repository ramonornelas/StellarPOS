...
export const CalcTotal: React.FC = () => {
  ...
  const getUnifiedSplitPaymentMethod = () => {
    ...
    const allSame = splitPayments.every(
      (p) => p.payment_method === firstMethod
    );
  ...
  const addNewOrder = async (paymentMethod: string) => {
    if (paymentMethod === "split") {
      ...
      if (
        remainingAmount > 0 &&
        splitPayments.some((payment) => payment.amount < 0)
      ) {
        const updatedSplitPayments = splitPayments.filter(
          (payment) => payment.amount >= 0
        );
        setSplitPayments(updatedSplitPayments);

        const messageCurrentDetails = `El total a pagar es de: ${formatCurrency(
          totalToPay
        )}. \nSe han pagado: ${formatCurrency(totalSplitPayments)}.`;
        const messageRequestToUser = `Se ha eliminado el Cambio Efectivo. \nConfirma la información antes de finalizar el pago.`;
        const message = `${messageCurrentDetails} \n\n${messageRequestToUser}`;
      ...
      if (remainingAmount > 0) {
        alert(
          "No se puede finalizar el pago. Aún resta " +
    ...
    getLastOrderId(dateString).then((lastOrderId) => {
      ...
      if (paymentMethod === "split") {
        orderReceivedAmount = splitPayments.reduce(
          (acc, payment) => acc + payment.amount,
          0
        );
        const { totalToPay } = calculateRemainingAmount();
        orderChangeAmount =
          orderReceivedAmount > totalToPay
  ...
  const calculateRemainingAmount = () => {
    const totalSplitPayments = parseFloat(
      splitPayments.reduce((acc, payment) => acc + payment.amount, 0).toFixed(2)
    );
    let totalToPay,
      remainingAmount = 0;
  ...
  const confirmClearCart = () => {
    ...
    // Reset any modal states if open
    if (open) {
      handleClose();
    }
    // Show success notification
    openSnackBarCartCleared();
    // Redirect to home if we're on the cart page (mobile view)
  ...
  const enableCartButton = () => {
    const isCartEmptyState = isCartEmpty(productsInCart);
    ...
    if (typeof amount === "number" && !Number.isNaN(amount)) {
      ...
      const newId =
        splitPayments.length > 0
          ? splitPayments[splitPayments.length - 1].id + 1
          : 1;
      const splitPayment = {
        id: newId,
        amount: amount,
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Enter" && !isButtonDisabled && remainingAmount <= 0) {
        e.preventDefault();
  ...
  return (
    <Paper className={classes["container-total"]} elevation={5} square>
            ...
            <Grid item xs={12} md={6}>
                      ...
                      >
                        <div>
                          <ToggleButtonGroup
                            ...
                            value={tipPercentage}
                            exclusive
                            onChange={(_, newTipPercentage) =>
                              handleTipPercentage(newTipPercentage)
                            }
              ...
              <Box sx={{ mt: 2 }}>
                <TextField
                  ...
                  margin="dense"
                  type="number"
                  helperText={showDiscount ? "" : "Ingrese el monto a agregar"}
                ...
                <Grid container spacing={2} justifyContent="center">
                  <Grid item xs={12} sm={4} sx={{ textAlign: "center" }}>
                    <Button
                      ...
                      disabled={
                        !canAddMorePayments || isButtonDisabled || showDiscount
                      }
                      sx={{
                        minHeight: 64,
                ...
                <Box sx={{ mt: 2, display: "flex", flexDirection: "column" }}>
                  {totalSplitPayments > 0 ? (
                    ...
                    >
                      ...
                      <br />
                      {"<"}--- Agrega un pago para ver el detalle aquí.
                    </Typography>
                  )}
                </Box>
              )}{" "}
            ...
            >
              ...
              >
                <Button
                  ...
                  sx={{
                    mt: 2,
                    boxShadow: highlightedFinalizar
                      ? "0 0 0 4px #81c784"
                      : undefined,
      ...
      >
        ...
        <DialogActions>
          <Button onClick={cancelClearCart} color="primary">
            Cancelar
          </Button>
